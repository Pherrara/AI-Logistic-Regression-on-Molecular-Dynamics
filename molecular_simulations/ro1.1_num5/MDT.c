/* 
   ...
   compile using "gcc -o outfile.exe  Codice_Drexel_MD_LennardJones_NVT_Andersen.c -lm -lgsl -lgslcblas"
...
void usage ( void ) {
  ...
  fprintf(stdout,"\t -Tb [real]\t\tSet bath temperature\n");
  ...
  fprintf(stdout,"\t -ecorr [real]\t\tSet energy correction\n");
...
int main ( int argc, char * argv[] ) {
  ...
  fprintf(stdout,"# L = %.5lf; rho = %.5lf; N = %i; rc = %.5lf\n",
	  L,rho,N,sqrt(rc2));
  fprintf(stdout,"# nSteps %i, seed %ld, dt %.5lf, T0 %.5lf, nu %.5lf\n",
